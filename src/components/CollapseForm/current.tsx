...
export type ConfigType = {
  ...
  total: number;
  getStatus: (
    info: { dsType: string; dsVersion: string },
    form: any
  ) => ReturnGetStatus;
...
const CollapseForm: React.FC<CollapseFormProps> = props => {
  ...
  const formatData = (keyArray: string[], formData: FormDataType = {}) => {
    const data = keyArray.map(item => ({
      value: item,
      ...formData[keyMap.get(item) as string]
    }));
    return data;
  };
  ...
  const collapseOnchange = (key: string | string[]) => {
    ...
    if (Array.isArray(key)) {
      if (key?.length > activeKey?.length) {
        const list = key?.filter(item => !activeKey.includes(item));
        config?.map((item: any) => {
        ...
        // 有值取消选中了
        const list = activeKey?.filter(item => !key.includes(item));
        config?.map((item: any) => {
          if (list?.includes(item?.key)) {
            item.used = item.used - 1;
          }
  ...
  const renderHeader = (node: ConfigType) => {
    ...
    const { disabled, tips, available } = node.getStatus(extra, formilyForm);
    return (
      <>
        <div>
          <Checkbox
        ...
        <div>
          <span>{tips}</span>
          <span>{available && `\uFF1A${node.used}/${node.total}`}</span>
        </div>
      </>
    );
  };
    ...
    return children.map((item, idx) => (
      <Form.Item
        ...
        {getFieldDecorator(
          ...
          noRulesWhenNotActive(String(parent.key), item.options)
        )(
  ...
  const renderChildren = () => {
    return config.map(child => {
      ...
      const { disabled } = child.getStatus(extra, formilyForm);
      return (
        <Panel
          ...
          {renderContent(child, child.children, disabled)}
        </Panel>
      );
    });
  };
